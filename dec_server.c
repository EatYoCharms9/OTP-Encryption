...
void setupAddressStruct(struct sockaddr_in* address, int portNumber){
    ...
    // Zeroing the struct, this avoids avoid garbage values
    memset((char*) address, '\0', sizeof(*address)); 
    address->sin_family = AF_INET; // Setting the address family to AF_INET for IPv4
    address->sin_port = htons(portNumber); // Converting the port number from host byte order to network byte order
    address->sin_addr.s_addr = INADDR_ANY; // Allowing the server to bind to any available interface
}

// Helper function to convert a character to its corresponding index
int charToIndex(char c) {
    // ' ' (space) is represented as 26; otherwise, subtract 'A' to get index (0-25)
    if (c == ' ') return 26;
    return c - 'A';
}
...
char indexToChar(int i) {
    // Normalizing negative indices by adding 27
    if (i < 0) i += 27;

    // 26 is represented as ' '; otherwise, add 'A' to index to get character
    if (i == 26) return ' ';
    return 'A' + i;
}

// Function to decrypt the given text using the given key, this is the section functionally different from the encrytion function.
void decrypt(char *text, const char *key, int length) {
    for (int i = 0; i < length; i++) {

        // Converting both text and key characters to indices and perform modular subtraction (this step undoes encrytion by performing subtraction instead of addition)
        int textIdx = charToIndex(text[i]);
        int keyIdx = charToIndex(key[i]);

        // Converting the result back to a character and store in text
        text[i] = indexToChar((textIdx - keyIdx + 27) % 27);
    }
}

// Function for handling the client-server communication process (implementation inspired by server.c from assignment doc)
void handleCommunication(int connectionSocket) {
    char buffer[BUFFER_SIZE]; // Buffer to store received ciphertext
    int charsRead; // Variable to keep track of the number of characters read
    ...
    // Read the key from the socket. This is necessary to get the key that will be used to decrypt the ciphertext.
    charsRead = recv(connectionSocket, key, keyLength, 0);
    if (charsRead < 0){
        error("ERROR reading from socket");
    }

    // Performing decryption using the received ciphertext and key
    decrypt(buffer, key, textLength);
...
// Main function (sets up the decryption server)
int main(int argc, char *argv[]){
    int connectionSocket;
    struct sockaddr_in serverAddress, clientAddress; // Structs for server and client addresses
    socklen_t sizeOfClientInfo = sizeof(clientAddress); // Size of the client address structure
    pid_t pid;
    ...
    // Enable the socket to begin listening. This is necessary to accept incoming connections.
