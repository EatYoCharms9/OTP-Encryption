...
int main(int argc, char *argv[]) {
    ...
    readFileContent(argv[1], ciphertext);
    readFileContent(argv[2], key);

    // Validating key length against plaintext length
    if (strlen(key) < strlen(ciphertext)) {
        fprintf(stderr, "CLIENT: ERROR, key length is shorter than ciphertext\n");
        exit(1);
    }
    ...
    // Send data to the server. This is necessary to transmit the ciphertext and key to the server.
    // The sendData function returns the number of bytes sent, which can be used to check if all data was sent successfully.

    // Sending the size of the ciphertext
    int ciphertextSize = strlen(ciphertext);
    if (sendData(socketFD, (char*)&ciphertextSize, sizeof(ciphertextSize)) < sizeof(ciphertextSize)) {
        fprintf(stderr, "CLIENT: WARNING: Not all ciphertext size data written to socket!\n");
    }

    // Send the ciphertext itself
    if (sendData(socketFD, ciphertext, ciphertextSize) < ciphertextSize) {
        fprintf(stderr, "CLIENT: WARNING: Not all ciphertext data written to socket!\n");
    }
    ...
    // Clearing the ciphertext buffer and receive encrypted text from the server
    memset(ciphertext, '\0', BUFFER_SIZE);
    charsRead = recv(socketFD, ciphertext, BUFFER_SIZE - 1, 0); 
    if (charsRead < 0) {
